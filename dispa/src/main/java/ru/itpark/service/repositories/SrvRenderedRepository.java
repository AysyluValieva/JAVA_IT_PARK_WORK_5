...
public interface SrvRenderedRepository extends JpaRepository<EventServicePatient, Integer> {
    ...
    @Query(nativeQuery = true, value = "SELECT ssr.id, sts.id as status, s.name, event_patient_id , mes.id service_id " +
            ...
            "join disp.md_event_service_status sts on sts.id = ssr.status " +
            "WHERE ssr.id = ?1")
